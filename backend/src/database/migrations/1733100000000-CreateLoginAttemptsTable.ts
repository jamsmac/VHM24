...
export class CreateLoginAttemptsTable1733100000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      CREATE INDEX idx_login_attempts_username_ip ON login_attempts(username, ip_address, attempted_at);
    `);

    // Create a partial index for failed attempts only (more efficient for queries)
    await queryRunner.query(`
      CREATE INDEX idx_login_attempts_failed ON login_attempts(username, attempted_at)
      WHERE success = FALSE;
    `);
  }
